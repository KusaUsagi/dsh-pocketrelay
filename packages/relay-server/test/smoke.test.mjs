...
test("host 注册 → 手机配对 → /d/ 转发", async () => {
  ...
  assert.ok(helloOk.pair.code.length === 6, "pair.code 为 6 位")
  ...
  // 3. host 作为响应端：收到 http-req → 回 head+chunk+end
  hostWs.addEventListener("message", (event) => {
    const frame = JSON.parse(event.data)
    if (frame.t !== "http-req") return
    hostWs.send(
      JSON.stringify({
        t: "http-head",
        id: frame.id,
  ...
  // 4. 手机 GET /d/<deviceId>/api/hello（带 cookie）→ 经 relay 转发到 host
